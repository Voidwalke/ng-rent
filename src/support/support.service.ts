...
export class SupportService {
  ...
  /** Все тикеты платформы (для суперадмина) */
  async findAllPlatform(filters?: { status?: TicketStatus; page?: number; limit?: number }) {
    const where: any = {};
    if (filters?.status) where.status = filters.status;
    const page = filters?.page || 1;
    const limit = filters?.limit || 20;
    const [data, total] = await Promise.all([
      this.prisma.supportTicket.findMany({
        ...
        take: limit,
        include: {
          user: { select: { id: true, fullName: true, email: true, tenant: { select: { name: true } } } },
          _count: { select: { messages: true } },
        },
        orderBy: { updatedAt: 'desc' },
      }),
  ...
  /** Тикет с сообщениями (для суперадмина, без tenantId) */
  ...
  /** Ответ от платформы (суперадмин) */
  async addPlatformReply(ticketId: number, userId: number, msg: string) {
    ...
    return this.prisma.supportMessage.create({
      data: { ticketId, senderId: userId, senderType: 'admin', message: msg },
    });
  }
